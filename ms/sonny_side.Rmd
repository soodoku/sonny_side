...
To do this, we collect the information that states in the United States have available online as it relates to businesses already registered in the state. This process is not trivial since all the states have different ways and rules for searching this data, and in some cases obtaining all the data is impossible. 

Once we collect the companies using the words son(s) and daughter(s), we estimate a ratio of these two numbers to measure the son to daughter bias per state. Finally, these data are correlated with other metrics in the state like GDP, population, geographic location, and political majority.
